...
| 2026-08-26 | **Richer-data audit.** Owner had already been paying for The Racing API; confirmed live (`/api/status`: paid mode, real API probe succeeded) and confirmed already reaching the training corpus (`pull-race-history.yml` → `data/normalized/`, running daily) — an earlier in-conversation claim that it "wasn't being used"/"never reaches training" was wrong on both counts and is corrected here. Confirmed via direct `wrangler d1 execute` against production that `RAILBIRD_DB` is real (1,907 horses, 8,448 workouts, 106,904 point-of-call rows) but frozen 2019-03-10 → 2023-08-31 — three years stale, predates the current meet, not usable as-is. Whatever off-band pipeline built that snapshot is not in this repo and can't be resumed from here; reviving it (not a new purchase) is the highest-leverage next step for workouts/trip-line data specifically. See `docs/HANDOFF.md` §14.9 for the full writeup. Also: `.github/workflows/deploy-worker.yml` added (owner's go-ahead given, per §11.5's own deferred condition) — `worker.js` now auto-deploys on push to `master` once `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID` repo secrets are added. Confirmed working same day (one secret-name typo fixed, then a green `workflow_dispatch` run). |
| 2026-08-26 | **New independent (confirmed non-Brisnet) PP-feed source, first real data covering the workout/trip-line gap.** Owner supplied a real, full past-performance export for the 2026-08-26 SAR card. Vendor identity confirmed directly with the owner before any code touched it, given this file's `docs/DATA_WISHLIST.md` own Brisnet history (see the two entries above) — this is a different, independent source. Field survey confirms no speed figures/class ratings at all (unlike Brisnet), but a real workout tab (12 works/horse) and full trip comments + running-line points of call (10 past races/horse) plus real per-meet post-position/pace-style win% — exactly items 6 ("Workouts") and the trip/trouble-line gap this file has tracked as missing all along. New `tools/parse_pp_feed.js` parser (verified field-by-field against the real file) + three new sub-scores in `scripts/lib/scoring.js` (`workoutSubScore`, `tripTroubleSubScore`, `dataDrivenBiasSubScore`). **Explicitly unfitted and not folded into the composite/picks** — a single day's card has no known race outcomes yet to validate weights against; this is plumbing + a live diagnostic only, pending enough accumulated days to backtest properly, same discipline as every other weight change this project has shipped. See `docs/HANDOFF.md` §14.10 for the full writeup. **Owner said they'll supply more of these going forward** — once several days/weeks accumulate with known results, a real fit becomes possible. |

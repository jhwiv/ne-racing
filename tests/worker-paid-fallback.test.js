...
async function callApi(pathname, env) {
  const worker = (await import(WORKER_URL)).default;
  const request = new Request('https://fake.test' + pathname, { method: 'GET' });
  return worker.fetch(request, env || PAID_ENV, {});
}

test('wrangler.toml DATA_SOURCE is free (TheRacingAPI inactive one-day stack)', () => {
  const m = WRANGLER.match(/^\s*DATA_SOURCE\s*=\s*"([^"]+)"/m);
  assert.ok(m, 'DATA_SOURCE assignment in wrangler.toml');
...
test('static SAR 2026-09-07 card is 12 races / ~128 runners with scratched:false', () => {
  ...
  for (const race of card.races) {
    assert.ok(Array.isArray(race.entries) && race.entries.length > 0, `R${race.race_number} entries`);
...
test('TRA 401 Subscription inactive falls back to Equibase scratches (dynamic)', async () => {
  ...
  try {
    ...
    assert.equal(body.scratches.length, 4);
    ...
    assert.ok(names.includes('Neigh Baby'));
    assert.ok(!names.includes('Still Running'), 'jockey notes must not be scratches');
    assert.ok(!names.includes('Newly Gelded'), 'gelding notes must not be scratches');
    assert.equal(body.scratches.filter((s) => s.horseName === 'Beira').length, 1, 'dedupe by race|pp|horse');
    assert.ok(body.scratches.every((s) => /^scratched$/i.test(s.reason)));
    assert.ok(body.paidFallback);
...
test('DATA_SOURCE=free uses Equibase scratches without TRA', async () => {
  ...
  try {
    const res = await callApi('/api/scratches?track=SAR&date=2026-09-07', {
      DATA_SOURCE: 'free', DEFAULT_TRACK: 'SAR', ALLOWED_ORIGIN: '*',
    });
    assert.equal(res.status, 200);
    const body = await res.json();
    assert.equal(body.source, 'equibase-live');

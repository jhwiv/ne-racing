...
// ── Piecewise-linear interpolation (shared, v2 continuous sub-scores) ───────
// v2.49.75: classSubScore/freshnessSubScore/trainerJockeySubScore_v2 squash a
// genuinely continuous underlying quantity (class-level diff, days since last
// race, jockey/trainer win%) into 4-6 discrete step values, throwing away real
// resolution the conditional logit fitter could otherwise use. This threw
// away information for no benefit -- the step boundaries were never
// backtested, just hand-picked, and a continuous function through the SAME
// anchor points (bucket midpoints, so the calibration intent is preserved)
// loses nothing the steps had and gains real resolution between and beyond
// them. `knots` = [[x0,y0],[x1,y1],...] sorted ascending by x; flat
// extrapolation beyond the first/last knot (matches the old step functions'
// own flat-beyond-the-extremes behavior).
function piecewiseLinear(x, knots) {
  if (!knots.length) return 50;
...
// v2: continuous replacement for classSubScore's discrete diff buckets.
// Knots are the OLD buckets' representative midpoints (e.g. diff=5 for the
// (0,10] bucket that used to flatly return 60), so the calibration intent is
// unchanged -- only the plateaus become smooth interpolation.
const CLASS_KNOTS_V2 = [[-15, 20], [-5, 35], [0, 50], [5, 60], [15, 75], [25, 90]];
function classSubScore_v2(horse, raceClassVal) {
  const lastClassVal = (horse.lastClass && CLASS_SCALE[horse.lastClass] != null)
    ? CLASS_SCALE[horse.lastClass]
    : raceClassVal;
  const diff = lastClassVal - raceClassVal;
  return piecewiseLinear(diff, CLASS_KNOTS_V2);
}
...
// v2.49.75: replaced the discrete 6-step bucket() with piecewiseLinear()
// through the same anchor points (bucket midpoints) -- continuous now, same
// calibration intent. See the piecewiseLinear() comment above classSubScore_v2.
const TJ_KNOTS_V2 = [[3, 20], [8, 35], [12, 50], [16, 65], [20, 80], [25, 95]];
function trainerJockeySubScore_v2(horse) {
  const jky = parseFloat(horse.jockeyPct) || 0;
  const trn = parseFloat(horse.trainerPct) || 0;
  const jScore = piecewiseLinear(jky, TJ_KNOTS_V2);
  const tScore = piecewiseLinear(trn, TJ_KNOTS_V2);
...
// v2: continuous replacement for the discrete daysSince buckets above -- a
// non-monotonic "bump" peaking at the old sweet-spot bucket's representative
// day count, falling off (asymmetrically, matching the old buckets' own
// asymmetry) on both sides. Knots are the old buckets' midpoints.
const FRESH_KNOTS_V2 = [[3, 50], [10, 65], [21, 80], [44, 55], [75, 35], [120, 20]];
function freshnessSubScore_v2(horse, today) {
  if (!horse.lastRaceDate) return 50;
  const ref = today ? new Date(today + 'T12:00:00') : new Date();
  const daysSince = Math.floor((ref - new Date(horse.lastRaceDate + 'T12:00:00')) / 86400000);
  return piecewiseLinear(daysSince, FRESH_KNOTS_V2);
}
...
function compositeForHorse(horse, race, paceCtx, bias, opts) {
  ...
  // v2.49.75: v2 gets the continuous (piecewise-linear) replacements for
  // class/freshness's discrete step buckets; v1 stays on the original
  // functions, unchanged, same as the existing tj/bias version split.
  const classScore = version === 'v2'
    ? classSubScore_v2(horse, raceClassVal)
    : classSubScore(horse, raceClassVal);
  ...
  const freshScore = version === 'v2'
    ? freshnessSubScore_v2(horse, opts.today)
    : freshnessSubScore(horse, opts.today);
...
function confidenceFor(scored) {
  ...
  global.RailbirdScoring = {
    ...
    loadFittedWeights: loadFittedWeights,
    DEFAULT_V2_WEIGHTS: DEFAULT_V2_WEIGHTS,
    classValueFor: classValueFor,
    parseOddsToNum: parseOddsToNum,
    expertPickMatchesHorse: expertPickMatchesHorse,
    countExpertPicks: countExpertPicks,
    speedSubScore: speedSubScore,
    classSubScore: classSubScore,
    classSubScore_v2: classSubScore_v2,
    paceSubScore: paceSubScore,
    isFrontRunning: isFrontRunning,
    isCloserStyle: isCloserStyle,
    trainerJockeySubScore_v1: trainerJockeySubScore_v1,
    trainerJockeySubScore_v2: trainerJockeySubScore_v2,
    biasSubScore_v1: biasSubScore_v1,
    biasSubScore_v2: biasSubScore_v2,
    ...
    fieldStrengthMultiplier: fieldStrengthMultiplier,
    probabilityNormalizeV1: probabilityNormalizeV1,
    probabilityNormalizeV2: probabilityNormalizeV2,
    attachOverlay: attachOverlay,
    CLASS_SCALE: CLASS_SCALE,
  };
})(typeof window !== "undefined" ? window : globalThis);
/* RAILBIRD_SCORING_INLINE_END */
